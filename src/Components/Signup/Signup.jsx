...
export default function Signup() {
  ...
  const handleSubmit = () => {
    const createUserPromise = new Promise((resolve)=>resolve(createUserWithEmailAndPassword(auth, user.email, user.password)));
    createUserPromise.then((createRes)=>{
      const storageRef = ref(storage, `${user.firstName}_${user.lastName}_${new Date().getTime()}`);
      return uploadBytesResumable(storageRef, displayPicture).then(()=> [ storageRef, createRes])
    })
    .then((res) => {
    ...
    <div className="unauthenticatedPage">
      ...
      <FormControl sx={{ m: 1, marginBottom: '16px' }} variant="standard">
        ...
        <div>
          <Button 
            variant="contained" 
            sx={{padding: '10px 20px', marginTop: '16px', marginBottom: '16px'}} 
            fullWidth
            onClick={()=>handleSubmit()}>Sign Up</Button></div>
        <div>OR</div>
        <div><Button variant="contained" sx={{padding: '10px 20px', marginTop: '16px', marginBottom: '12px'}} fullWidth>Sign Up using Google</Button></div>
